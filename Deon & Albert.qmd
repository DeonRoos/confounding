...
highlight_row <- which(top_models$formula == "y ~ x + z")

top_models |>
  arrange(desc(`Number of times ranked best (lowest AIC)`)) |>
  kbl(caption = "Top model rankings across simulations") |>
  row_spec(highlight_row, bold = TRUE, background = "#FFFF99")

x_effect_summary |>
  arrange(desc(`Median AIC rank`)) |>
  kbl(caption = "Estimated effect of x") |>
  row_spec(highlight_row, bold = TRUE, background = "#FFFF99")
...
highlight_row <- which(top_models$formula == "y ~ z")
